 ...
 */
	...
	<?php if ( is_single() ) : // navigation links for single posts ?>
		...
		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '<i class="icon-left-open"></i>', 'Previous post link', 'phylo' ) . '</span> %title' ); ?>
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<footer>
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'phylo' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author .vcard -->
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'phylo' ); ?></em>
					<br />
				<?php endif; ?>

				<div class="comment-meta commentmetadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"><time datetime="<?php comment_time( 'c' ); ?>">
					<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'phylo' ), get_comment_date(), get_comment_time() ); ?>
					</time></a>
					<?php edit_comment_link( __( 'Edit', 'phylo' ), '<span class="edit-link">', '<span>' ); ?>
 ...
 */
function phylo_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so phylo_categorized_blog should return true
		return true;
	} else {
		// This blog has only 1 category so phylo_categorized_blog should return false
		return false;
	}
}

/**
 * Flush out the transients used in phylo_categorized_blog
 */
function phylo_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'all_the_cool_cats' );
}
add_action( 'edit_category', 'phylo_category_transient_flusher' );
add_action( 'save_post', 'phylo_category_transient_flusher' );
