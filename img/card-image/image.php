 ...
 * @return void
 */
function create_image( $width=250, $height=392,  $color, $merge = array(),$version=0, $file ) {

		$color_rgb = rgb2hex2rgb($color);

		if( !GD_VERSION || !is_writable(  get_template_directory()."/img/generated-card-images/" ) )
			return false;
        ...
        foreach( $merge as $item ) {
          if( in_array( $item, array('desert', 'forest', 'fresh-water', 'grasslands', 'ocean', 'tundra', 'urban') ) ) {
            $merge_array[] = $item;
          }
        }
        ...
        if($count == 3)
        	$correction = 1;
		if($count > 0):
			foreach($merge_array as $insert):

				unset($addition_image);
				// load the image
       // if ( file_exists( get_template_directory()."/img/card-image/source/$insert-$count.png" ) ) {
				$addition_image = imagecreatefrompng( get_template_directory()."/img/card-image/source/$insert-$count.png");
			 //}
		...
		imagepng($im, get_template_directory().$file );
